...
export const convertHeicToJpegIfNecessary = async (file) => {
  ...
  // 1. Try fast native browser decoding (takes <50ms if supported natively)
  try {
  ...
  // 2. Fallback to heic2any for engines without native HEIC decoder
  try {
 ...
 * Uses fast createImageBitmap API with HTML5 canvas for ultra-fast background decoding.
 * Reduces file sizes by 80-95% while maintaining high visual clarity.
 */
export const compressImage = (file, options = {}) => {
  return new Promise((resolve, reject) => {
    ...
    createImageBitmap(file)
      ...
      .catch(() => {
        // Legacy fallback via FileReader & HTMLImageElement
