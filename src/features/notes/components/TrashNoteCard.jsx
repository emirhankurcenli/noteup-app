...
const TrashNoteCard = ({
  ...
  // Guard: deletedAt null ise güvenli default
  const daysLeft = note.deletedAt
    ? 30 - Math.floor((Date.now() - note.deletedAt) / (1000 * 60 * 60 * 24))
    : 30;
  ...
  const handleCardClick = () => {
    if (isSelectMode) {
      ...
      // Çöp kutusundaki notu açmadan önce geri yükle, sonra düzenleyiciyi aç.
      // Aksi hâlde düzenlemeler deletedAt ile birlikte kaydedilerek not
      // çöpte kalmaya devam eder.
