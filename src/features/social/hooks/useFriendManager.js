...
import { sanitizeFriendCode } from '@shared/utils/securityUtils';
...
const useFriendManager = ({
  ...
  const pollFriendRequests = useCallback(async () => {
    ...
    // Fetch ALL relevant requests (pending + accepted) in a SINGLE optimized query
    try {
      const { data: allRequestsData, error: reqError } = await supabase
        .from('friend_requests')
        .select('*')
        .or(`to_code.eq.${myCode},from_code.eq.${myCode}`)
        .in('status', ['pending', 'accepted']);

      if (!reqError && allRequestsData) {
        const pendingData = allRequestsData.filter(r => r.status === 'pending');
        const acceptedData = allRequestsData.filter(r => r.status === 'accepted');

        // Collect all distinct related codes across both pending and accepted requests in ONE set
        const allRelatedCodes = Array.from(new Set(
          allRequestsData.map(r => r.from_code === myCode ? r.to_code : r.from_code).filter(Boolean)
        ));

        // Single batch fetch for all required user profiles
        const { data: allProfiles } = allRelatedCodes.length > 0
          ? await supabase
              .from('profiles')
              .select('friend_code, photo_url, name')
              .in('friend_code', allRelatedCodes)
          : { data: [] };

        const profilesMap = new Map((allProfiles || []).map(p => [p.friend_code, p]));
        ...
        pendingData.forEach(remoteReq => {
          ...
          const otherProf = profilesMap.get(otherCode);
        ...
        if (acceptedData.length > 0) {
          const validFriendCodesSet = new Set(acceptedData.map(r => r.from_code === myCode ? r.to_code : r.from_code));

          let friendsUpdated = false;
          // Keep only friends that are still accepted in Supabase
          ...
          acceptedData.forEach(req => {
            ...
            const friendProf = profilesMap.get(friendCode);
  ...
  useEffect(() => {
    ...
    pollingRef.current = setInterval(pollFriendRequests, 60_000);
